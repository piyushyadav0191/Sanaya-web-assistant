...
window.addEventListener("load", function() {
    // alert("Activate sanaya");
    setTimeout(() => {
        document.querySelector(".preloader").style.display = "none";
    }, 1000);
    speak('Activating sanaya');
    speak('going online');
    wishMe();
});
...
function speakThis(message) {
    ...
    if (message.includes('hi') || message.includes('hello') || message.includes('hii') || message.includes('hey')) {
        ...
        const finalText = 'My name is Sanaya';
        speech.text = finalText;
...
function wishMe() {
    ...
    var hr = day.getHours();
    if (hr >= 0 && hr < 12) {
        speak('Good Morning from Master Piyush');
    } else if (hr == 12) {
        speak('Good Noon from Master Piyush');
    } else if (hr >= 12 && hr <= 17) {
        speak('Good Afternoon from Master Piyush');
    } else {
        speak('Good Evening from Master Piyush');
    }
}